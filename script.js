...
(() => {
  ...
  function initLogin(){
    ...
    form.addEventListener('submit',e=>{
      ...
      err.hidden=true;
      if(CLASSES.includes(cls)&&p===cls){setSession(cls);location.href='index.html'}
  ...
  async function initSite(){
    ...
    const js=$('#jurusanSelect'); if(js&&JURUSAN[session])js.value=session;
    $('#uploadForm')?.addEventListener('submit',async e=>{
      ...
      try{
        await put({title:String(data.get('title')||'').trim(),uploader:String(data.get('uploader')||'').trim(),category:data.get('category'),type:data.get('type'),jurusan:data.get('jurusan'),status:'pending',createdAt:Date.now(),blob:f});
        e.currentTarget.reset(); js.value=session; preview.innerHTML='';
